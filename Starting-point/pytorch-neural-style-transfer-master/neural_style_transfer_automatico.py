...
def build_loss(neural_net, optimizing_img, target_representations, content_feature_maps_index, style_feature_maps_indices, config):
   ...
   # tv_loss = utils.total_variation(optimizing_img)
    ...
    wandb.log({
        ...
        "tv_loss": config['tv_weight'] * tv_loss,
...
def neural_style_transfer(config):
    ...
    if config['optimizer'] == 'adam':
        ...
        for cnt in range(config["num_of_iterations"]):
            ...
            with torch.no_grad():
                print(f'Adam | iteration: {cnt:03}, total loss={total_loss.item():12.4f}, content_loss={config["content_weight"] * content_loss.item():12.4f}, style loss={config["style_weight"] * style_loss.item():12.4f}, tv loss={config["tv_weight"] * tv_loss:12.4f}')
        ...
        def closure():
            ...
            if total_loss.requires_grad:
                total_loss.backward()
            with torch.no_grad():
                print(f'L-BFGS | iteration: {cnt:03}, total loss={total_loss.item():12.4f}, content_loss={config["content_weight"] * content_loss.item():12.4f}, style loss={config["style_weight"] * style_loss.item():12.4f}, tv loss={config["tv_weight"] * tv_loss:12.4f}')
                utils.save_and_maybe_display(optimizing_img, dump_path, config, cnt, config["num_of_iterations"], should_display=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--content_weight", type=float, help="weight factor for content loss", default=30e6)#1e5 10e5
    parser.add_argument("--style_weight", type=float, help="weight factor for style loss", default=3e4)#3e4
